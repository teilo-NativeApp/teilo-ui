    ...
    groupData.events.map((event, index)=>{
      ...
      const newEvent = (
        <View key={`e-${index}`} style={dashboardStyles.rowFlex} >
          <CustomText title={`${moment(event.date).format("DD.MM.YY")}`} p bold style={{width:75}}/>
  ...
  return (
    <View style={dashboardStyles.margin}>
      ...
      <View style={generalStyles.roundedBox}>
        ...
        {upcoming2}
        {upcoming3}
        {upcoming1.length == 0 && upcoming2.length == 0 && upcoming3.length == 0 ? <CustomText title="Nothing planned for the next days!" p/> : null}
      </View>
    </View>
  )
}
